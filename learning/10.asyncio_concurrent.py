...
async def fetch(session, param):
    async with session.get(f'https://deelay.me/{param*1000}/ \
        http://httpbin.org/get?a={param}') as resp:
...
def req():
    ...
    p1, p2 = 2, 1
    return [
        session.get(f'https://deelay.me/{p1*1000}/ \
            http://httpbin.org/get?a={p1}').json(),
        session.get(f'https://deelay.me/{p2*1000}/ \
                http://httpbin.org/get?a={p2}').json()
    ]
